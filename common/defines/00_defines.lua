...
NGame = {
	...
	GAME_SPEED_SECONDS = { 100.0, 0.25, 0.1, 0.04, 0.0 }, -- game speeds for each level. Must be 5 entries with last one 0 for unbound
	...
	MAX_EFFECT_ITERATION = 100,					-- maximum allowed iteration for loop effects
	MAX_SCRIPTED_LOC_RECURSION = 20,				-- max recursion for scripted localizations
...
NDiplomacy = {
	...
	EMBARGO_COST = 50,										-- One-time cost
	REVOKE_EMBARGO_COST = 0,								-- Cost to remove an existing embargo
	EMBARGO_THREAT_THRESHOLD = -1,						-- Target-generated threat threshold to allow embargo (affected by modifiers)
...
NCountry = {
	...
	POPULATION_YEARLY_GROWTH_BASE = 0,			-- basic population growth per year, used for monthly manpower gain
	...
	PROVINCE_SCORE_MULTIPLIER = 0,				-- Based on number of controlled provinces.
	NUCLEAR_PRODUCTION_SCALE = 365,					-- +1 nuclear_production gives 1 nuke per year
	NUCLEAR_BOMB_DROP_WAR_SUPPORT_EFFECT_MAX_INFRA = 0.35, -- Reduce enemy national war support on nuking a province, the value scales with infrastructure up to this number
	...
	SPECIAL_FORCES_CAP_BASE = 0.2,					-- Max ammount of special forces battalions is total number of non-special forces battalions multiplied by this and modified by a country modifier
	SPECIAL_FORCES_CAP_MIN = 40,					-- You can have a minimum of this many special forces battalions, regardless of the number of non-special forces battalions you have, this can also be modified by a country modifier
	...
	GIE_EXILE_TROOPS_DEPLOY_TRAINING_MAX_LEVEL = 5, 	--Max XP exile troops can receive from training
...
NResistance = {
	...
	GARRISON_MANPOWER_LOST_BY_ATTACK = 0.005, 	-- Ratio of manpower lost by garrison at each attack on garrison (this number will be reduced by the hardness of garrison template)
...
NProduction = {
	...
	ANNEX_FIELD_EQUIPMENT_RATIO = 1,	-- How much equipment from deployed divisions will be transferred on annexation
	...
	MIN_POSSIBLE_TRAINING_MANPOWER = 15000000,	-- How many deployment lines minimum can be training
	...
	MIN_LICENSE_ACTIVE_DAYS = 0,						-- Min days for license to be active
...
NTechnology = {
	...
	BASE_RESEARCH_POINTS_SAVED = 56.0,		-- Base amount of research points a country can save per slot.
	BASE_YEAR_AHEAD_PENALTY_FACTOR = 2.5,		-- Base year ahead penalty
...
NBuildings = {
	...
	MAX_SHARED_SLOTS = 35,				-- Max slots shared by factories
...
NMilitary = {
	...
	UNIT_EXPERIENCE_PER_TRAINING_DAY = 0.015,
	...
	TRAINING_MAX_DAILY_COUNTRY_EXP = 0.16,			-- Maximum army XP gained per day from training
	...
	PLANNING_DECAY = 0.01,
	PLAYER_ORDER_PLANNING_DECAY = 0.005,				-- Amount of planning lost due to player manual order
	PLANNING_GAIN = 0.02,
	...
	PLAN_SPREAD_ATTACK_WEIGHT = 4.1,				-- The higher the value, the less it should crowd provinces with multiple attacks.
	...
	NUKE_MAX_DAMAGE_PERCENT = 1.2,					-- Minimum damage from nukes as a percentage of current strength/organisation
	...
	COMMANDER_LEVEL_UP_STAT_WEIGHTS = {7, 5, 3, 4}, -- level up stat random base weights attack, defense, planning, logistics
	...
	NEW_COMMANDER_RANDOM_PERSONALITY_TRAIT_CHANCES = {  -- chances to gain a personality trait for new generals
		0.8, -- 50% for first trait
		0.4,  -- 15% for second trait after that
		0.05,
		0.01,
	},
	...
	UNIT_LEADER_MODIFIER_COOLDOWN_ON_GROUP_CHANGE = 3,		-- time in days for a unit leader to regain its modifiers
...
NAir = {
	...
	AIR_WING_XP_LEVELS = { 20, 40, 60, 80, 100, 140, 180, 220, 260, 300, 390, 480, 570, 660, 750, 780, 810, 840, 870, 900 }, 						--Experience needed to progress to the next level
	...
	AIR_WING_XP_LOSS_REDUCTION_OVER_FRIENDLY_TERRITORY_FACTOR = 0.1, 	--Reduction on XP loss over friendly territory
...
NNavy = {
	...
	RESOURCE_ORIGIN_PRIORITY = 3,									-- Default convoy priority for resources shipped internally
	...
	NAVAL_INVASION_PREPARE_HOURS = 120,								-- base hours needed to prepare an invasion
	...
	SUPPLY_NEED_FACTOR = 0,										    -- multiplies supply usage
	...
	PRIDE_OF_THE_FLEET_LOST_TEMP_MODIFIER_DURATION = 50,			-- duration for temp modifiers that you get when you lose your pride of the fleet
...
NAI = {
	...
	BASE_RELUCTANCE = 5, 						-- Base reluctance applied to all diplomatic offers
	...
	DIPLOMATIC_ACTION_BREAK_SCORE = -50, 		-- AI must score a diplomatic action less than this to break it off
	...
	DIPLOMACY_FACTION_SAME_IDEOLOGY_MAJOR = 50, -- AI bonus acceptance when being asked about faction is a major of the same ideology
	...
	PLAN_ATTACK_MIN_STRENGTH_FACTOR_HIGH = 0.4,
	...
	PLAN_ACTIVATION_PLAYER_WEIGHT_FACTOR = 2.0,		-- AI countries will hold on activating plans if player controlled countries have plans in the same location.
	...
	NEW_LEADER_EXTRA_CP_FACTOR = 1.5,					-- Country must have at least this many times extra command power to get new admirals or army leaders
	SCARY_LEVEL_AVERAGE_DEFENSE = -0.7,                 -- average front defense modifier to make it consider it as a PITA to go for
	ATTACK_HEAVILY_DEFENDED_LIMIT = 0.7,				-- AI will not launch attacks against heavily defended fronts unless they consider to have this level of advantage (1.0 = 100%)
	...
	INVASION_DISTANCE_RANDOMNESS = 245,					-- This higher the value, the more unpredictable the invasions. Compares to actual map distance in pixels.
	...
	XP_RATIO_REQUIRED_TO_RESEARCH_WITH_XP = 1.5,		-- AI will at least need this amount of xp compared to cost of a tech to reserch it with XP
	...
	FRONT_EVAL_UNIT_ACCURACY = 2.0,								-- scale how stupid ai will act on fronts. 0 is potato
	FRONT_EVAL_UNIT_AIR_SUP_IMPACT = 2.0,                       -- scale how good the AI thinks air superiority is for units
...
NOperatives = {
	...
	BUILD_INTEL_NETWORK_DAILY_XP_GAIN = 1.5,
	QUIET_INTEL_NETWORK_DAILY_XP_GAIN = 1,
	...
	COUNTER_INTELLIGENCE_DAILY_XP_GAIN = 0.115,
	...
	OPERATIVE_BASE_BOOST_IDEOLOGY = 0.15,					-- Base amount of daily ideology drift provoked by an operative
	...
	DIPLOMATIC_PRESSURE_DAILY_XP_GAIN = 0.155,
...
NCharacter = {
	...
	ADVISOR_PROMOTION_COST = 0,	-- Cost to promote someone to advisor
